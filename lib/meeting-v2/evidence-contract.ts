...
export function draftReadiness(agenda: GateItem[], investigations: Array<{ agendaItemId: string; modelName?: string | null; usageJson?: string | null }>,
  ...
  for (const item of agenda) {
    ...
    const verdict = findings.find(v => v.code === "ai_verdict" || v.code === "item_not_discussed");
    ...
    if (verdict?.detailsJson) {
      try { if (JSON.parse(verdict.detailsJson).needsHumanReview === true) problems.push(`${item.title}: human review is required.`); }
