...
async function run() {
  ...
  function strictWallTouchesUnit(s1, s2, polygon, toleranceM = 0.25) {
    ...
    for (const t of [0.2, 0.5, 0.8]) {
      ...
      if (pointInPolygon({ x: pt.x, y: pt.z }, poly2D)) {
        return true;
      }
      ...
      for (let i = 0; i < polygon.length; i++) {
        ...
        const u = ((pt.x - e1.x) * edx + (pt.z - e1.z) * edz) / lenSq;
        if (u < 0.02 || u > 0.98) continue;

        const projX = e1.x + u * edx;
        const projZ = e1.z + u * edz;
        const perpDist = Math.hypot(pt.x - projX, pt.z - projZ);

        if (perpDist <= toleranceM) {
          const sdx = s2.x - s1.x;
          const sdz = s2.z - s1.z;
          const dot = Math.abs(sdx * edx + sdz * edz);
          const mag = Math.hypot(sdx, sdz) * Math.hypot(edx, edz);
          if (mag > 1e-9 && dot / mag >= 0.85) {
            return true;
          }
        }
      }
  ...
  console.log(`Planarized into ${vertices.length} vertices and ${edges.size} edges`);
  ...
  for (const key of edges) {
    ...
    worldEdges.push({
      ...
      len,
      touches822: strictWallTouchesUnit(start, end, u822Poly),
      startPt,
      endPt,
    });
  }
  ...
  console.log(`\nStrict edges touching 822: ${touching822.length}`);
