...
import {
  ...
  orgEvidenceFieldLabel,
  orgEvidenceMatchReasonLabel,
...
function highlightTypeForField(field: OrgEvidenceField): OrgHighlightType {
  if (field === "phone") return "phone";
  if (field === "website") return "website";
  if (field === "organization_role") return "organization_role";
...
function MarkedBody({
  ...
  const cls = ORG_HIGHLIGHT_CLASS[highlightTypeForField(field)];
  ...
  return (
    <span className="mt-1 flex flex-wrap gap-1">
      {reasons.map((reason) => (
        ...
        >
          {orgEvidenceMatchReasonLabel(reason)}
        </span>
      ))}
    </span>
  );
    ...
    <li className="border-b border-slate-100">
      ...
      {open ? (
        <div className="border-t border-slate-100 bg-slate-50 px-4 py-3">
          ...
          {message ? (
            <MarkedBody text={bodyText} needle={needle} field={field} />
          ) : null}
        </div>
      ) : null}
    </li>
...
export function OrgEvidenceSidePanel({ target, onClose }: Props) {
  const [evidence, setEvidence] = useState<OrgEvidencePayload | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    setEvidence(null);
    setError(null);
  }, [target?.organizationId, target?.field, target?.value]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/organizations/evidence?${params.toString()}`)
      .then(async (response) => {
        const data = (await response.json()) as {
          evidence?: OrgEvidencePayload;
          error?: string;
        };
        if (!response.ok) {
  ...
  return (
    <>
      ...
      <aside
        role="dialog"
        aria-modal="true"
        aria-labelledby="org-evidence-panel-title"
        ...
        <header className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-200 px-5 py-4">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
              {orgEvidenceFieldLabel(target.field)} evidence
            </p>
            <h2
              id="org-evidence-panel-title"
              className="mt-1 text-lg font-semibold text-slate-900"
            >
              {evidence?.value ?? target.value}
            </h2>
            <p className="mt-1 text-sm text-slate-600">
              For {target.organizationName}
            </p>
          </div>
          <button
            type="button"
          ...
          {evidence ? (
            evidence.matchedCount === 0 ? (
              <p className="p-4 text-sm text-slate-500">
                No emails found where this {orgEvidenceFieldLabel(target.field).toLowerCase()} was
                extracted as an organization field.
              </p>
            ) : (
              <>
                <p className="border-b border-slate-100 px-4 py-2 text-xs text-slate-500">
                  ...
                  {loading ? " · updating…" : ""}
                  {" · org-card or highlight extractions"}
                </p>
                <ul>
