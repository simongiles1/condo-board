...
- **Meetings V2 segment gold standard** — Segmenter compare can toggle **Gold standard** mode: a selectable lab walk × edge reference on the left, your gold labels on the right, and agenda concepts in the sidebar. Select a leaf item, then click the first and last cues of that discussion on the gold column. Labels persist on the meeting. Completed lab cells show **Gold *n*%** mean time-span overlap versus those labels. **Run minutes pipeline from gold** writes the spans onto agenda evidence, clears post-extract work, and resumes evidence → investigation → validation as if segmentation were perfect.
...
- **Meetings V2 unofficial 4.D overflow** — Transcript-only matters after the last package **items for discussion** leaf (for example 4.D.m after 4.D.l) are moved to extraTopics and nested under **Ad-hoc items** (4.E). Official 4.D letters are frozen from the board package so extras no longer extend D.

- **Meetings V2 gold-standard span editing** — A leaf that is not labeled yet can be clicked inside another item's transcript box (identical or nested ranges such as 4.A.1 and 4.A.2). Resize handles hide until that leaf has a span, so they no longer swallow the first click. Abutting spans keep separate top/bottom handles; hovering an edge outlines that item's range and thickens its amber edge so coincident lines are distinguishable. Agenda-concept hover fill uses the same rounded box as the selected amber ring, with the ring drawn on top so it is not covered or inset from the hover.

- **Meetings V2 gold-standard segmenter** — Opening Segmenter compare no longer fails when extract cost accounting cannot load (missing snapshot table or Supabase session pooler at max clients); the transcript still opens and cost estimates are skipped. Gold-standard mode also no longer stretches each transcript cue to the full panel height, so the whole conversation is visible, and the agenda concepts column is twice as wide so titles wrap less. While choosing a span, hover now draws a connected amber outline (top cap on the start cue, sides through the middle, bottom cap on the hover end) instead of faint disconnected rings. That preview uses inset strokes so cue text does not shift when the highlight appears. Gold-standard cue rows stay flush (no extra padding at a span handoff), so a boundary does not open a gap on both transcript columns. For a labeled leaf, drag the top or bottom edge of its drawn segment box (north–south resize cursor on a wide hit strip, including the agenda pill row) to move the start or end cue; works on any labeled span, not only when that leaf stays selected in the sidebar. Resize hit-testing uses the speaker box itself, not the padded row wrapper. Clicking to replace a span now clears any existing span for that agenda item instead of merging them. In speech-to-text transcripts, adjacent speaker turns often have 0.1s–1.5s of audio bleed/cross-talk; cue assignment to section overlays and sequence ranges now requires either the cue's midpoint to lie within the span or at least 50% duration overlap, preventing adjacent turns from bleeding into the span upon click or causing the resize handle to jump or lag behind the pointer. Agenda concepts with a gold span use a teal row background; the active leaf gets an inset amber ring so selection does not shift layout.
...
- **Recurring documents — “Series not found” after discovery** — When Find recurring types finished, the sidebar could still point at a series row id that discovery had replaced, so the file list showed a red error even though the types loaded. The page now remaps the selection by title and ignores stale member fetches.
...
- **Gold-standard compare viewer (mobile)** — Below the `lg` breakpoint, the highlight legend, meeting title, and compared timestamp move into a header more menu; overlap coverage badges stay visible on all sizes. Side-by-side rows use constrained columns and word breaking so the compare view scrolls vertically only.

### Fixed

- **Gold-standard compare highlight paint** — Diff backgrounds no longer cover leading or trailing whitespace on a marked span (for example the space before “Chair: the AGM” after boundary repair inserts a separator space).

- **Gold-standard compare outline nav** — The left concept list shows titles only again; coverage and paired/split chips stay in the main compare rows, not duplicated in the sidebar.
