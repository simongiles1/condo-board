...
3. **Resolve facts:** resolve material contractors, amounts, approvals, dates, and conditions before generating prose. Each candidate cites a source ID and a verifiable quotation. Keep package proposals, prior approvals, and current decisions separate. A package candidate cannot override a genuinely different figure in the meeting evidence. A spoken amount that only drops the digits below the thousands place is the one matching package figure, not a conflict. A package proposal cannot establish a current decision except in that truncation case. Unresolved selections block drafting.
