/**
 * Fuzzy similarity for organization display names.
 * Strips legal suffixes so "Acme Inc" ≈ "Acme Ltd" can cluster together.
 *
 * Keep this module free of server/db imports — it is used from client UI.
 */

/** Pairwise score at or above this joins orgs into the same duplicate group. */
export const ORG_NAME_FUZZY_THRESHOLD = 0.78;

/** Same rules as normalizeOrgNameKey — inlined so this file stays client-safe. */
...
/**
 * Normalize an org name for fuzzy compare: lowercase alphanumerics, then drop
 * trailing legal-form tokens (inc, ltd, llc, …) while leaving the core name.
 */
export function canonicalizeOrgNameForFuzzyMatch(
  name: string | null | undefined,
): string {
  const base = normalizeOrgNameKey(name);
  if (!base) return "";
...
/**
 * Similarity of two already-canonicalized org name strings in [0, 1].
 * Uses the best of token Jaccard, character trigram Jaccard, and edit ratio.
 */
export function orgCanonicalNameSimilarity(a: string, b: string): number {
  if (!a || !b) return 0;
  if (a === b) return 1;
...
/** Similarity of two raw org names after canonicalization. */
export function orgNameSimilarity(
  a: string | null | undefined,
  b: string | null | undefined,
): number {
  return orgCanonicalNameSimilarity(
    canonicalizeOrgNameForFuzzyMatch(a),
    canonicalizeOrgNameForFuzzyMatch(b),
  );
}
 ...
 */
export function orgNamesBestSimilarity(
  ...
  const left = [
    ...new Set(
      leftNames
        .map((n) => canonicalizeOrgNameForFuzzyMatch(n))
        .filter(Boolean),
    ),
  ];
  const right = [
  ...
  for (const a of left) {
    for (const b of right) {
      const score = orgCanonicalNameSimilarity(a, b);
 ...
 */
export function orgFuzzyBlockingKeys(
  ...rawNames: Array<string | null | undefined>
): string[] {
  const keys = new Set<string>();
  for (const raw of rawNames) {
    const canonical = canonicalizeOrgNameForFuzzyMatch(raw);
